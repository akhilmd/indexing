...
func RunPauser(pauseMgr *PauseServiceManager, task *taskObj, master bool, pauseToken *PauseToken) {
	...
	// TODO: Move logic from run to handlers for PauseStateTokens
...
func (p *Pauser) generatePauseStateTokens() (map[string]*PauseStateToken, error) {
	...
	if err != nil || len(indexerUuids) < 1 {
		logging.Errorf("Pauser::generatePauseStateTokens: Error getting indexer node UUIDs: err[%v]" +
	...
	for _, uuid := range indexerUuids {
		...
		if oldPst, ok := psts[pstId]; ok {
			err = fmt.Errorf("pst with pstId[%s] already exists, pst[%v]", pstId, oldPst)
			logging.Errorf("Pauser::generatePauseStateTokens: PSTId Collision: err[%v]", err)
			return nil, err
		}
...
// processStateTokens is metakv callback, not intended to be called otherwise
func (p *Pauser) processStateTokens(kve metakv.KVEntry) error {

	if kve.Path == buildMetakvPathForPauseToken(p.pauseToken) {
		// Process PauseToken

		logging.Infof("Pauser::processStateTokens: PauseToken path[%v] value[%s]", kve.Path, kve.Value)
		...
		// Process PauseStateTokens

		if kve.Value != nil {
			pstId, pst, err := decodePauseStateToken(kve.Path, kve.Value)
			if err != nil {
				logging.Errorf("Pauser::processStateTokens: Failed to decode PauseStateToken. Ignored.")
				return nil
			}
			...
			logging.Infof("Pauser::processStateTokens: Received empty or deleted PauseStateToken path[%v]",
				kve.Path)
...
func (p *Pauser) processPauseStateToken(pstId string, pst *PauseStateToken) {
	logging.Infof("Pauser::processPauseStateToken pstId[%v] pst[%v]", pstId, pst)
	if !p.addToWaitGroup() {
		logging.Errorf("Pauser::processPauseStateToken: Failed to add to pauser waitgroup.")
		return
	}
	...
	// call, as metakv will call parent processStateTokens again for each state change.
	var processed bool
...
func (p *Pauser) processPauseStateTokenAsFollower(pstId string, pst *PauseStateToken) bool {
	...
	logging.Infof("Pauser::processPauseStateTokenAsFollower: pstId[%v] pst[%v]", pstId, pst)

	if pst.PauseId != p.task.taskId {
		logging.Warnf("Pauser::processPauseStateTokenAsFollower: Found PauseStateToken[%v] with Unknown " +
			"PauseId. Expected to match local taskId[%v]", pst, p.task.taskId)
...
func (p *Pauser) startPauseUpload(pstId string, pst *PauseStateToken) {
	start := time.Now()
	logging.Infof("Pauser::startPauseUpload: Begin work: pstId[%v] pst[%v]", pstId, pst)
	defer logging.Infof("Pauser::startPauseUpload: Done work: pstId[%v] pst[%v] took[%v]",
		pstId, pst, time.Since(start))
